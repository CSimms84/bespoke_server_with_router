...
void test_add_route() {
    ...
    // Check that the route was added by simulating a request to "/hello"
    int fds[2];
    pipe(fds);
...
void test_handle_request() {
    // Set up a route and simulate a request to it
    add_route("/test", dummy_hello_handler);

    // Create a pipe to simulate client socket
    int fds[2];
    pipe(fds);
    int client_socket = fds[1];

    // Write a dummy HTTP request to the client socket
...
void test_dns_server() {
    ...
    unsigned char query[] = {
        ...
        // Question section
        0x07, 'e', 'x', 'a', 'm', 'p', 'l', 'e', 
        0x03, 'c', 'o', 'm', 0x00, // QNAME (example.com)
        0x00, 0x01, // QTYPE (A)
        0x00, 0x01  // QCLASS (IN)
    };
